...
# https://huggingface.co/datasets/hhy3/ann-datasets/raw/main/siftsmall-128-euclidean.hdf5
NUM_DATA = 10000
DATA_FILE = "siftsmall-128-euclidean.hdf5"
DIST_TYPE = "l2"
NUM_QUERY = 1
TOPK = 5
...
DATA_URL = f"https://huggingface.co/datasets/hhy3/ann-datasets/resolve/main/{DATA_FILE}"
...
def download():
  DATA_DIR = pjoin(RES_DIR, DATA_FILE[:-5])
  if not os.path.exists(RES_DIR):
    os.makedirs(RES_DIR)
  if not os.path.exists(DATA_DIR):
    os.makedirs(DATA_DIR)

  data_path = pjoin(DATA_DIR, DATA_FILE)
...
def run_gpu_inference2(topk=TOPK, index_file="cuhnsw.index", ef_search=300):
  print("=" * BARRIER_SIZE)
  # data_path = pjoin(RES_DIR, DATA_FILE)
  base_path = pjoin(RES_DIR, DATA_FILE[:-5])
  data_path = pjoin(base_path, DATA_FILE)
  data_dir = pjoin(base_path, f"q{NUM_QUERY}")
  index_path = pjoin(data_dir, index_file)
  LOGGER.info("gpu inference on %s with index %s", data_path, index_path)
  ch0 = CuHNSW(OPT)
  LOGGER.info("load model from %s by cuhnsw", index_path)
  ch0.load_index(index_path)
  ...
  h5f = h5py.File(data_path, "r")
  data = h5f["train"][:, :].astype(np.float32)
  queries = h5f["test"][:NUM_QUERY, :].astype(np.float32)
  h5f.close()
  if NRZ:
    data /= np.linalg.norm(data, axis=1)[:, None]
  ...
  nns, distances, found_cnt = ch0.search_knn(queries[:NUM_QUERY], topk, ef_search, data_dir)
