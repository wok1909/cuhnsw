...
def run_cpu_inference(loop, num_query, topk=5, ef_search=300, index_file=INDEX_FILE,
                      evaluate=True, num_threads=8):
  print("=" * BARRIER_SIZE)
  print("(ns)")
  data_path = pjoin(RES_DIR, DATA_FILE)
  index_path = pjoin(RES_DIR, f"q{num_query}_top5_l5_ef300", index_file)
...
#   print(f"load {index_path} by hnswlib")
  ...
  for i in range(loop):
    ...
    el0 = time.time() - start
#   print(f"elapsed for processing {num_queries} queries computing top@{topk}: {el0:.16e} sec",)
    # print(f"{el0:.16e} sec")
    # print(f"{el0 * 1e9:.4f}")
    result_times.append(el0)
  hl0.print_elapsed_time(loop)
  ...
  if evaluate:
    ...
    for _pred_nn, _gt_nn in zip(labels, neighbors):
        ...
        accs.append(acc)
    # print(f"accuracy mean: {np.mean(accs):.4e}, std: {np.std(accs):.4e}")
    return el0, np.mean(accs)
  return el0

if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(f"Usage: python {sys.argv[0]} <num_query> <num_threads>")
        sys.exit(1)

    num_query = int(sys.argv[1])
    num_threads = int(sys.argv[2])

    print(f"Query: {num_query}, nThreads: {num_threads}")
    run_cpu_inference(loop=100, num_query=num_query, num_threads=num_threads)