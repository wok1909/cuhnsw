...
def run_cpu_inference(num_query, topk=5, ef_search=300, index_file=INDEX_FILE,
...
#   print(f"load {index_path} by hnswlib")
  ...
  for i in range(warmup):
    labels, _ = hl0.knn_query(queries[num_query * i : num_query * (i + 1)], k=topk, num_threads=num_threads)
  start = time.time()
  labels, _ = hl0.knn_query(queries[num_query * warmup : num_query * (warmup + 1)], k=topk, num_threads=num_threads)
  el0 = time.time() - start
  # print(f"Outer time: {el0}")
  
  return el0

if __name__ == "__main__":
    if len(sys.argv) != 4:
        print(f"Usage: python {sys.argv[0]} <num_query> <num_threads> <warmup>")
        sys.exit(1)
    ...
    print(f"Query: {num_query}, nThreads: {num_threads}, warmup: {warmup}")
    run_cpu_inference(num_query=num_query, num_threads=num_threads, warmup=warmup)